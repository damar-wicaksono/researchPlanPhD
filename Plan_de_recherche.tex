...
During early days of reactor safety analysis, prediction using model of power plant behavior during transient, off-normal or accident conditions, is approached with high-degree of conservatism. 
...
Therefore, an accurate prediction of two-phase flow transient behavior is required for safety analysis of nuclear power plant under accident conditions.
Such need calls for physical thermal-hydraulics models with realistic modeling assumptions which are also backed by experimental data.
...
The code is designed to be a comprehensive tool to simulate realistically wide range of transients foreseen in nuclear power plant operation.
It was developed using the current best knowledge of flow processes expected to happen during the transients.
...
The resolution of non-equilibrium conditions in two-phase flows is necessary for simulating several important flow phenomena during transient.

The validity of the two-fluid model widely implemented in thermal-hydraulics system codes relies on the proper modeling of the transfer terms between phases and each phase to the boundary wall. 
...
As best-estimate analysis uses less conservative assumptions, and even the best available knowledge of two-phase flow processes in nuclear power plant during transient is still limited, best-estimate analysis has to be complemented with uncertainty analysis.
The ultimate goal of the analysis is to associate the code prediction with its uncertainty. 
